...
(function ($) {
  ...
  const allowChainId = 1;
  const contractAddress = "0x22E2259bbA5b63d0Bd12fFc85A29C6B0DcB10830";
  let connectedWallet = "";
  let connectedChain;
  let funds = 0;
  let contribute = 0;
  ...
  const errorSpan2 = document.getElementById("error-2");
  ...
  ethereum.on("chainChanged", (chainId) => {
    if (Number(chainId) !== allowChainId) {
      errorSpan2.innerHTML = "Please connect to Ethereum Mainnet";
      errorSpan2.style.display = "block";
      connectedChain = "";
    } else connectedChain = chainId;
  });

  function onAccountChanges(accounts) {
    ...
    connectBtn.innerHTML = `${shortAccount}`;
  ...
  async function getContribute() {
    ...
    var fundsContract = new web3.eth.Contract(fundsAbi, contractAddress);
    const balance = await fundsContract.methods
      .contribute(connectedWallet)
      .call();
    contribute = Web3.utils.fromWei(balance, "ether");
    //get 3 number after decimal point remove 0 at the end if exist
    contribute = Number(parseFloat(contribute).toFixed(2));
    document.querySelector("#deposited").innerText = contribute;
    return balance;
  }
  ...
  async function getOwnerBalance() {
    var web3 = new Web3(ethereum);
    var fundsContract = new web3.eth.Contract(fundsAbi, contractAddress);
    const balance = await fundsContract.methods.getOwnerBalance().call();
    funds = Web3.utils.fromWei(balance, "ether");
    //get 3 number after decimal point
    funds = Number(parseFloat(funds).toFixed(2));
    document.querySelector("#totalRaised").innerText = funds;
    return balance;
  }

  document.querySelector("#deposited").innerText = contribute;
  document
    .querySelector("#depositInput")
    .addEventListener("input", function () {
      var value = parseFloat(this.value);
      var errorSpan = document.getElementById("error");

      if (isNaN(value) || value < 0.1) {
        errorSpan.innerHTML = "The minimum value is 0.1";
        errorSpan.style.display = "block";
      } else if (value > 0.5) {
        errorSpan.innerHTML = "The maximum value is 0.5";
        errorSpan.style.display = "block";
      } else {
  ...
  document
    ...
    .addEventListener("click", async function () {
      var value = parseFloat(document.querySelector("#depositInput").value);
      ...
      if (Number(window.ethereum.chainId) !== allowChainId) {
        errorSpan2.innerHTML = "Please connect to Ethereum Mainnet";
        errorSpan2.style.display = "block";
      ...
      fundsContract.methods
        ...
        .on("receipt", function (receipt) {
          successSpan.innerHTML = "Deposit success";
        ...
        .on("error", function (error, receipt) {
          if (error.code === 4001) {
            errorSpan2.innerHTML = "User rejected transaction";
            errorSpan2.style.display = "block";
            loading.style.display = "none";
            return;
          }
          errorSpan2.innerHTML = "Deposit failed";
          errorSpan2.style.display = "block";
          loading.style.display = "none";
        });
    });
  ...
  var retinaLogos = function () {
    var retina = window.devicePixelRatio > 1 ? true : false;
    if (retina) {
      $(".header__logo").find("img").attr({
        src: "assets/images/logo/logo@2x.png",
        width: "180",
        height: "30",
      });
      $(".logo_footer").find("img").attr({
